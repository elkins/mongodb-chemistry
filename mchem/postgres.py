...
@click.option(
    ...
    "-d",
    default="mchem",
    envvar="MCHEM_POSTGRES_DB",
    help="PostgreSQL database name (default: mchem).",
)
@click.option(
    "--user",
    "-u",
    default="root",
    envvar="MCHEM_POSTGRES_USER",
    help="PostgreSQL username (default: root).",
)
@click.option(
    "--password", "-p", default=None, envvar="MCHEM_POSTGRES_PASSWORD", help="PostgreSQL password."
)
...
def load(conn):
    ...
    cur.execute(
        "drop table if exists biotherapeutics, drug_mechanism, activities, assays, assay_parameters, compound_records, compound_properties, molecule_hierarchy, ligand_eff, predicted_binding_domains, molecule_synonyms, docs, formulations, molecule_atc_classification cascade;"
    )
    cur.execute(
...
@click.option(
    ...
    "-f",
    default="m2",
    type=click.Choice(["m2", "m3", "m2l2048", "m2l512", "m3l2048", "m3l512"]),
    help="Fingerprint type (default: m2).",
)
...
def profile(conn, sample, fp, threshold):
    ...
    for i, mol_id in enumerate(mol_ids[:100]):
        log.debug(f"Query molecule {i + 1} of {len(mol_ids)}: {mol_id}")
        ...
        cur.execute(
            "select %s from rdk.fps where molregno = %s",
            (
                AsIs(fp),
                molregno,
            ),
        ...
        start = time.time()
        cur.execute(
            "select molregno from rdk.fps where %s%%%s",
            (
                AsIs(fp),
                qfp,
            ),
...
def samplesim(conn, sample, threshold, fp):
    ...
    click.echo(f"Fingerprint: {fp}, Threshold: {threshold}")
    cur = conn.cursor()
    mol_ids = sample.read().strip().split("\n")
    cur.execute("set rdkit.tanimoto_threshold=%s;", (threshold,))
    for i, mol_id in enumerate(mol_ids[:100]):
        ...
        for mrn in results:
            cur.execute(
                "select chembl_id from chembl_id_lookup where entity_id = %s and entity_type = 'COMPOUND'",
                (mrn,),
            )
