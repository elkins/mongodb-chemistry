...
def test_screening(
    ...
    for i, qmol in enumerate(mols):
        remain = screen(
            qmol, fingerprinter, fp_collection, threshold, count_collection, reqbits, counts
        )
        log.debug(f"Query molecule {i + 1} of {len(mols)}: {remain} remaining")
...
def test_ideal(
    ...
    log.info(f"Testing ideal screening: fp: {fingerprinter.name} Threshold: {threshold}")
    result = {
        "fp": fingerprinter.name,
        "threshold": threshold,
        "remaining": [],
    ...
    for i, qmol in enumerate(mols):
        remain = len(
            similarity_search(qmol, fingerprinter, fp_collection, threshold, count_collection)
        )
        log.debug(f"Query molecule {i + 1} of {len(mols)}: {remain} remaining")
        result["remaining"].append(remain)
    result["median_remaining"] = np.median(result["remaining"])
